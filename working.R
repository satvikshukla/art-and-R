...
col <- stat[, 1]
df <- data.frame(matrix(ncol = 62, nrow = 0))
df.colnames <-  c("art", "year",#"artist", "movement", "year",
...
for (i in 1:length(col)) {
  str <- col[i]
    j <- (i - 1) %% 61
    if (j == 0) {
      #col[i] <- str_extract(col[i], "([a-z]|[A-Z]|-|[0-9]);")
      col[i] <- str_extract(str, ";([a-z]|[A-Z]|[0-9]|-)*;")
      v <- c(v, col[i])
    }
    #if (j == 1) {
    # col[i] <- str_extract(col[i], "; (([a-z]|[A-Z])*-*([a-z]|[A-Z])){3,}")
    # print("here")
    # v <- c(v, col[i])
    #}
    #else if (j == 2) {
    # col[i] <- str_extract(col[i], "; ([a-z]|[A-Z]){2};")
    # v <- c(v, col[i])
    #}
    if (j == 0) {
